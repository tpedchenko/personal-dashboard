...
export default function KrakenPage() {
  ...
  useEffect(() => {
    startTransition(async () => {
      const [key, secret] = await Promise.all([getSecret("kraken_api_key"), getSecret("kraken_api_secret")]);
  ...
  function handleSave() {
    startTransition(async () => {
      if (!apiKey.startsWith("••••")) await setSecret("kraken_api_key", apiKey.trim());
      if (!apiSecret.startsWith("••••")) await setSecret("kraken_api_secret", apiSecret.trim());
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4 space-y-4">
        ...
        <div className="space-y-1"><Label>API Key</Label><PasswordInput value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Kraken API key" /></div>
        <div className="space-y-1"><Label>API Secret</Label><PasswordInput value={apiSecret} onChange={(e) => setApiSecret(e.target.value)} placeholder="Kraken API secret" /></div>
      ...
      <Card className="p-4 space-y-2">
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          <p>1. Register at <a href="https://kraken.com" className="underline">kraken.com</a></p>
